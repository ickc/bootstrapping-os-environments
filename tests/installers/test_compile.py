...
_COMPILED_MODULES = ["clifton", "code", "codex", "gh", "mamba", "mamba_env", "pixi", "sman", "zim"]
...
def test_compiled_modules_list_is_complete():
    """Every compiled script in install/ must be covered by _COMPILED_MODULES.

    Guards against adding a new installer (and its compiled artifact) while
    forgetting to add it to the freshness/self-containment checks below.
    """
    discovered = {path.stem for path in (REPO_ROOT / "install").glob("*.py") if _COMPILE_MARKER in path.read_text()}
    assert discovered == set(_COMPILED_MODULES), (
        f"compiled scripts in install/ ({sorted(discovered)}) differ from "
        f"_COMPILED_MODULES ({sorted(_COMPILED_MODULES)})"
    )
...
def test_checked_in_script_is_up_to_date(module):
    ...
    assert artifact == compile_module(module), (
        f"install/{module}.py is stale; regenerate with `pixi run compile-{module.replace('_', '-')}`"
    )
