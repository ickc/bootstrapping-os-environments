...
while IFS= read -r executable || [[ -n ${executable} ]]; do
    ...
    for path_pattern in "${executable}" "*/${executable}" "/usr/bin/${executable}" "/bin/${executable}" "/usr/sbin/${executable}" "/sbin/${executable}"; do
        ...
        result=$(dnf provides "${path_pattern}" 2> /dev/null | awk '/^[^[:space:]]+[[:space:]]+:/ { print; exit }' || true)
    ...
    if [[ -n ${package_info} ]]; then
        ...
        clean_package_name=${package_info%%-[0-9]*}
        clean_package_name=${clean_package_name%%.el*}
...
found_packages=$(wc -l < "${DNF_FILE}")
missing_executables=$(wc -l < "${MISSING_FILE}")
echo "Found packages: ${found_packages} (saved to ${DNF_FILE})"
echo "Missing executables: ${missing_executables} (saved to ${MISSING_FILE})"
