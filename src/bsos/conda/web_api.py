...
def _pixi_version_spec(version: str) -> str:
    """Map a conda ``=version`` pin from the CSV to a pixi version spec.

    An empty version means unpinned (``*``); a bare version keeps conda's
    ``=``-prefix (starts-with) semantics via a trailing ``.*``; anything
    already carrying an operator is passed through unchanged.
    """
    if not version:
        return "*"
    if version[0] in "<>=!~":
...
def _pixi_dependency_entry(package: CondaPackage) -> str:
    """Render one ``name = spec`` line for a pixi.toml dependency table."""
    spec = _pixi_version_spec(package.version)
    if package.channel:
        return (
            f"{json.dumps(package.name)} = {{ version = {json.dumps(spec)}, channel = {json.dumps(package.channel)} }}"
        )
    return f"{json.dumps(package.name)} = {json.dumps(spec)}"
...
def _write_pixi_manifest(
    ...
    *,
    ...
    channels: list[str],
    archs: list[str],
    envs: dict[str, tuple[list[str], dict[str, list[str]]]],
) -> None:
    """Write a pixi.toml declaring one feature + named environment per env.

    *envs* maps environment name to ``(common_deps, per_arch_deps)`` where the
    dependency entries are pre-rendered ``name = spec`` lines: *common_deps*
    for packages available on every arch, *per_arch_deps* for packages only
    available on a subset (emitted under ``[feature.<env>.target.<arch>]``).
    """
    lines: list[str] = [
        "# Generated by bsos.conda.web_api — do not edit by hand.",
        "[workspace]",
        f"name = {json.dumps(workspace_name)}",
        f"channels = {json.dumps(channels)}",
        f"platforms = {json.dumps(archs)}",
    ]
    for env_name, (common, per_arch) in envs.items():
        lines.append("")
        lines.append(f"[feature.{json.dumps(env_name)}.dependencies]")
        lines.extend(common)
        for arch in archs:
            if per_arch.get(arch):
                lines.append("")
                lines.append(f"[feature.{json.dumps(env_name)}.target.{json.dumps(arch)}.dependencies]")
                lines.extend(per_arch[arch])
    lines.append("")
    lines.append("[environments]")
    for env_name in envs:
        lines.append(f"{json.dumps(env_name)} = {{ features = [{json.dumps(env_name)}], no-default-feature = true }}")
...
def _lock_and_convert(manifest: Path, env_names: list[str], out_dir: Path) -> None:
    ...
    for env_name in env_names:
        with tempfile.TemporaryDirectory(prefix="bsos-conda-lock-") as tmp:
            ...
            produced = sorted(Path(tmp).glob("*.yml")) + sorted(Path(tmp).glob("*.yaml"))
            if len(produced) != 1:
                raise RuntimeError(f"Expected exactly one conda-lock file for {env_name!r}, got {produced}")
            shutil.move(str(produced[0]), out_dir / f"{env_name}-lock.yml")
    ...
    *,
    ...
    if lock:
        ...
        for version in versions:
            ...
            common: list[str] = []
            per_arch: dict[str, list[str]] = {arch: [] for arch in archs}
            if python:
                common.append(f'"python" = {json.dumps(f"{version}.*")}')
            for package in packages.packages:
                ...
                if not supported:
                    continue
                entry = _pixi_dependency_entry(package)
                if len(supported) == len(archs):
                    common.append(entry)
                else:
                    for arch in supported:
                        per_arch[arch].append(entry)
