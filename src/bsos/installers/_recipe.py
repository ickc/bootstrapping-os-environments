...
class Pinned(VersionSpec):
    ...
    overrides it; the leading ``v`` is stripped from the override so the URL
    template's hardcoded ``v{version}`` pattern stays consistent.
    """

    value: str

    def resolve(self, override: Optional[str] = None) -> str:
        return (override or self.value).lstrip("v")
